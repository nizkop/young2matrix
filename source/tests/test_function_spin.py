...
###   unit test spin function calculation within tableau   ###
...
trials = [
    ...
    {"input": [(1,2,3)], "spin_input": ["α", "α", "α"], "expected_sqrt": 1,
            "expected": [{"value": ['α1', 'α2', 'α3'], "sign": Sign.PLUS}],
            "error_message": "symmetrizing wrong (> 2)"},
    {"input": [(1,2,3)], "spin_input": ["α", "α", "β"], "expected_sqrt": 3,
            "expected": [{"value": ['α1', 'α2', 'β3'], "sign": Sign.PLUS},
                         {"value": ['α3', 'α2', 'β1'], "sign": Sign.PLUS},
                         {"value": ['α1', 'α3', 'β2'], "sign": Sign.PLUS}],
            "error_message": "symmetrizing wrong (> 2)"},
    {"input": [(1, 2, 3)], "spin_input": ["β", "β", "α"], "expected_sqrt": 3,
            "expected": [{"value": ['β1', 'β2', 'α3'], "sign": Sign.PLUS},
                         {"value": ['α1', 'β2', 'β3'], "sign": Sign.PLUS},
                         {"value": ['β1', 'α2', 'β3'], "sign": Sign.PLUS}],
            "error_message": "symmetrizing wrong (> 2)"},
    ...
    {"input": [(1,2),(3,)], "spin_input": ["α", "β", "β"],"expected_sqrt": 2,
            "expected": [{"value": ['α1', 'β2', 'β3'], "sign": Sign.PLUS},
                         {"value": ['α3', 'β2', 'β1'], "sign": Sign.MINUS}],
            "error_message": "anti- & symmetrizing wrong"},
    {"input": [(1, 2), (3,)], "spin_input": ["α", "α", "β"], "expected_sqrt": 2,
            "expected": [{"value": ['α1', 'α2', 'β3'], "sign": Sign.PLUS},
                         {"value": ['α3', 'α2', 'β1'], "sign": Sign.MINUS}],
            "error_message": "anti- & symmetrizing wrong"},
    {"input": [(1,3),(2,)], "spin_input": ["α", "β", "α"], "expected_sqrt": 2,
            "expected": [{"value": ['α1', 'β2', 'α3'], "sign": Sign.PLUS},
                         {"value": ['β1','α2','α3'], "sign": Sign.MINUS}],
            "error_message": "anti- & symmetrizing wrong"},
    {"input": [(1, 3), (2,)], "spin_input": ["α", "β", "β"],
        "expected": [{"value": ['α1', 'β2', 'β3'], "sign": Sign.PLUS},
                     {"value":  ['β1', 'α2', 'β3'], "sign": Sign.MINUS}],
...
for i in trials:
    ...
    for calculated in s.function.parts:
        calculated_part = "".join(sorted(calculated.get_list_of_parts())).replace("{","")
        calculated_part = calculated_part.replace("}","").replace("_","")
        try:
    ...
    assert len(expected) == len(s.function.parts), f"{general_error} - {i['error_message']}: {calculated_part} missing"
    # comparison of the square roots (underneath the fraction bar):
    assert s.function.get_normalization_factor()["1/sqrt"] == i["expected_sqrt"], \
        f"{general_error}: wrong normalization factor for \"{i['error_message']}\""
