...
def get_max_number_of_signs_in_equation(eq: str) -> int:
    ...
    formula = re.sub(r"\\quad", "q", formula)
    formula = re.sub(r"\\qquad", "qq", formula)
    ...
    formula = re.sub(r"\\bra{", "<", formula)
    formula = re.sub(r"\\ket{",">",formula)
    ...
    relevant_characters = re.findall(r"[a-zA-Z0-9ß+\-=:&()*\_<>]", formula)
